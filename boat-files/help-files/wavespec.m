...
%`wavespec` 函数通过多种经典的波浪谱模型，根据输入的参数和频率向量，计算波浪谱密度

%Bretschneitder谱：通过参数A和B定义，公式为S(w)=A*w^(-5)*exp(-B/(w^4))。
%Pierson-Moskowitz谱：基于20米高度处的平均风速计算参数A和B，公式与Bretschneitder谱类似。
%ITTC-Modified Pierson-Moskowitz谱（Hs,T0）：基于显著波高和模态周期计算参数A和B。
%ITTC-Modified Pierson-Moskowitz谱（Hs,T1）：基于显著波高和平均波周期计算参数A和B。
% ITTC-Modified Pierson-Moskowitz谱（Hs,Tz）：基于显著波高和零交叉周期计算参数A和B。
% JONSWAP谱（Vwind10,Fetch）：基于10米高度处的风速和风区长度计算参数，公式较为复杂，包含两个部分S1和S2。
% JONSWAP谱（gamma,Hs,w0）：基于显著波高、模态频率和尖峰因子计算参数，公式与第6种类似，但参数计算方式不同。
% Torsethaugen谱（Hs,w0）：基于显著波高和模态频率，是一种双峰谱，适用于涌浪和成长海浪。
...
%     
% SpecType and Par =[p1,p2,p3,...pn]:  八种不同的波浪谱模型
